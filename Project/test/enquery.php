...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
   ...
   $paymment_done =0;
    ...
    // Check the package type and calculate price accordingly
    if ($package == 'package1') {
        $package = 'Same Day Return';
        $returnDate = $dateOfJourney;
        $numberOfRooms = 'Not Required ';
        
        // Calculate price for Same Day Return based on conditions
        ...
        if ($daysDifference == 1) {
            ...
            // Calculate price for One Night Two Days based on conditions
            if ($numberOfPersons <= 2) {
                $price = $numberOfPersons * 4000;
            } elseif ($numberOfPersons <= 5) {
                $price = $numberOfPersons * 6000;
            } else {
                $price = $numberOfPersons * 5000;
            }
            ...
            // Calculate price for other packages based on conditions
            // Adjust the logic as per your pricing requirements
        } else {
    ...
    $sql = "INSERT INTO enquiry_register (name, email, mobile, package, dateOfJourney, returndate, numberOfPersons, selectcar, numberOfCars, pickuplocation, dropLocation, price,payment_done) 
            VALUES ('$name', '$email', '$mobile', '$package', '$dateOfJourney', '$returnDate', '$numberOfPersons', '$selectCar', '$numberOfCars', '$pickupLocation', '$dropLocation', $price,$paymment_done)";

    // Execute the query
    if ($conn->query($sql) === TRUE) {
        // Redirect to future-trip.php after successful insertion
        header('Location: enquiry-register.php?sort=timestamp');
        exit();
    } else {
        // Set the error flag and display the specific error message
        $insertError = true;
        echo '<div class="alert alert-danger" role="alert">Error: ' . $conn->error . '</div>';
    }
} else {
    // If it's not a POST request, clear the stored form data in session
...
<a href="index.php" class="btn btn-primary btn-block Dashboard">
    ...
    <div class="container mt-5" style="border-radius:20px;margin-bottom:170px;">
        <div class="row justify-content-center">
            <form id="invoiceForm" action="enquery.php" method="post" class="col-sm-8 col-md-6 needs-validation"
                enctype="multipart/form-data" novalidate>
                <h2 class="text-center mb-4">Enquiry Register &#128512;
</h2>
                ...
                <div class="mb-3">
                    <label for="package" class="form-label">Select Package: <span style="color: red;"> *</span></label>
                    <select class="form-control" id="package" name="package" required onchange="toggleDateFields()">
                        <option value="package1">Same Day Return</option>
                        <option value="package2">Another Package</option>
                    </select>
                </div>

                <div class="mb-3" id="dateOfJourneyDiv" style="display: none;">
                    <label for="dateOfJourney" class="form-label">Date of Journey:<span style="color: red;"> *</span> </label>
                ...
                <div class="mb-3">
                    <label for="numberOfPersons" class="form-label">Number of Persons: <span style="color: red;"> *</span></label>
